...
public class IdFilter<ID extends Entity, E extends Entity> {
    ...
    public JoinResult<ID, E> join(ID p_key, List<E> entities) {
        if (p_key == null || entities.isEmpty()) {
            throw new IllegalArgumentException("Parameters cannot be empty or null");
        }

        return new JoinResult<>(p_key, entities.stream().filter(e -> switch (e) {
            ...
            case Customer c -> {
                if (p_key instanceof Adress a) {
                    yield c.fk_adress().equals(a.id());
                } else {
                    throw new IllegalArgumentException("Invalid entity type, " + e.getClass().getSimpleName() + " has no foreign key of type : " + p_key.getClass().getSimpleName());
                }
            }
            case Order o -> {
                if (p_key instanceof Customer c) {
            ...
            case OrderDetailed od -> {
                if (p_key instanceof ShoeDetailed sd) {
                    yield od.fk_sko_detailed().equals(sd.id());
                } else if (p_key instanceof Order o) {
                    yield od.fk_order().equals(o.id());
                } else {
                    throw new IllegalArgumentException("Invalid entity type, OrderDetailed has no foreign key of type : " + p_key.getClass());
                }
            }
            case OutOfStock oos -> {
                if (p_key instanceof ShoeDetailed sd) {
